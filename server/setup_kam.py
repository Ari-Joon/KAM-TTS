...
Steps (idempotent, safe to re-run):
  ...
  3. Register the Chrome native-messaging host so the extension's power
     button can launch the server (runs register_host.py). Needs no extension
     id, since manifest.json pins a key that fixes the id on every machine.
...
def _register_host():
    ...
    This used to need the extension id, which Chrome only assigns once the
    extension has been loaded, so setup could not know it. manifest.json now
    pins a public key and Chrome derives the id from that instead, so the id is
    the same everywhere and register_host.py defaults to it.
    ...
    if r.returncode != 0:
        ...
        print("Only the dashboard power button depends on this, so the rest of")
        print("setup is unaffected. Start the server with: python server.py")
